...
def load_data(work_dir='../Data', fermentation_number=8, data_file='data.xlsx', x_cols=[], y_cols=[]):
    #Load data from .xlsx files
    
    var_to_keep = [*x_cols, *y_cols]
    data = pd.read_excel(io=join(work_dir, str(fermentation_number), data_file), usecols=var_to_keep).dropna(how='all').to_dict()
...
def data2sequences(x, ws=20, stride=10):
    ...
    x = np.pad(array=x, pad_width=((0, compute_padded_length(len(x), ws, stride)), (0, 0)), mode='edge')
...
def plot_od600_curve(preds, labels, dir):
    ...
    plt.legend()
    plt.savefig(join("logs", dir, "od600pred.jpg"))
