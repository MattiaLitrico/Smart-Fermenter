...
class FermentationData(Dataset):
    def __init__(self, work_dir='../Data', train_mode=True, y_var=['od_600'], ws=20, stride=10):
        self.work_dir = work_dir
        self.train = train_mode
        
        #lists of number of fermentations for training and testing
        self.train_fermentations = [11, 12, 14, 16, 17, 19, 20, 22, 23, 24]
        self.test_fermentations = [8]

        #variables with cumulative values
        self.cumulative_var = ['dm_o2', 'dm_air', 'dm_spump1', 'dm_spump2', 'dm_spump3', 'dm_spump4']
        #variables with binary values
        self.binary_var = ['induction']
        
        #input variables
        self.x_var = ['m_ph', 'm_ls_opt_do', 'm_temp', 'm_stirrer', 'dm_o2', 'dm_air', 'dm_spump1', 'dm_spump2', 'dm_spump3', 'dm_spump4', 'induction']
        #output variable
        self.y_var = y_var

        #Using fermentation 16 for computing normalisation parameters     
        self.fermentation_norm_number = 16
        self.X_norm, _ = self.load_data(fermentation_number=self.fermentation_norm_number)
    ...
    def load_data(self, fermentation_number=None):
        ...
        if fermentation_number is not None:
            data = utils.load_data(work_dir=self.work_dir, fermentation_number=fermentation_number, data_file='data.xlsx', x_cols=self.x_var, y_cols=self.y_var)
            X.append(data[0])
            Y.append(data[1])
        ...
        if self.train:
            for fn in self.train_fermentations:
                data = utils.load_data(work_dir=self.work_dir, fermentation_number=fn, data_file='data.xlsx', x_cols=self.x_var, y_cols=self.y_var)
                X.append(data[0])
                Y.append(data[1])
        else:
